...
void gui_pages()
{
  ...
  my_gif_set_spi(img3, "P:/example.gif"); // animated GIF
 ...
 // lv_label_set_recolor(table, true);
  ...
  // test scrolled clock PAGE 4 with background
  LV_LOG_USER("Page 4");
  LV_IMG_DECLARE(background);
  lv_obj_t *imbackground = lv_img_create(tv4);
  lv_img_set_src(imbackground, &background);
  lv_obj_align(imbackground, LV_ALIGN_CENTER, 0, 0);
  buildClock(tv4, 20, 60);

  // Test Purtherminal
  lv_obj_t *ipurth = lv_img_create(tv5);
#ifndef LEPTON
  LV_IMG_DECLARE(purthermal); // see file purthermal.c generated by LVGL tools
