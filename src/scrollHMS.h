...
lv_obj_t* createTile( lv_obj_t *tv, int x, int y) {
  ...
  lv_obj_set_style_text_color(sc, lv_palette_main(LV_PALETTE_YELLOW), 0);
...
void createDblPts(lv_obj_t *tv, int x, int y){
  ...
  lv_obj_set_style_bg_opa(wgtPP, LV_OPA_TRANSP, 0);
  lv_obj_set_style_text_color(wgtPP, lv_palette_main(LV_PALETTE_YELLOW), 0);
}
