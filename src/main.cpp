...
// ESP32X uses pins 19=MISO, 23=MOSI, 16=SCK, and 5=SS.
...
void setup() {
 ...
 //  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0); //disable brownout detector
   ...
   // flirController.printModuleInfo();
    ...
    button2.attachClick([]() { gui_switch_page(); });

    LV_LOG_WARN("Start LCD startingTask");
    // Get Time
    ...
    delay(800);
    LV_LOG_WARN("Start GUI startingTask");
...
void loop() {
    ...
    if ( (timeinfo.tm_min % 2==0) && timeinfo.tm_sec == 30) {
      if ((wifiSt != WL_CONNECTED) ) {
        wifiLost++;
        if (wifiLost == 2 ) {
          LV_LOG_WARN("WiFi connection is lost. cnt:%d minutes", wifiLost);
          WiFi.disconnect();
        }
        if (wifiLost == 4) {
          if (WiFi.reconnect()) {
            LV_LOG_WARN("WiFi reconnect OK (%d).", wifiLost);
            wifiLost = 0;
          }
        }
    ...
    updateTime(WiFi.localIP().toString());
        ...
        text2 +=  '\t'; text2 += ESP.getPsramSize() / 1024;
        text2 +=  " kB\t"; text2 += ESP.getFlashChipSize() / 1024;
        text2 +=  " kB\t"; text2 += WiFi.macAddress().c_str();
        text2 +=  '\t'; text2 += WiFi.localIP().toString().c_str();
        text2 +=  '\t'; text2 += getDate(false);
        text2 +=  '\t'; text2 +="#0000ff Bruno #";
        text2 +=  '\t';
